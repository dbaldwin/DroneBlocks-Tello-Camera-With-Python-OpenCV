...
Blockly.prompt = function(message, defaultValue, callback) {
  CustomDialog.show('Create Variable', message, {
    showInput: true,
    showOkay: true,
    onOkay: function() {
...
CustomDialog.show = function(title, message, options) {
  ...
  dialogDiv.innerHTML =
      '<header class="customDialogTitle" style="font-weight: bold"></header>' +
      '<p class="customDialogMessage"></p>' +
      (options.showInput ? '<div><input id="customDialogInput"></div>' : '') +
      '<div class="customDialogButtons">' +
      (options.showCancel ? '<button id="customDialogCancel" class="waves-effect waves-light btn-flat">Cancel</button>': '') +
      (options.showOkay ? '<button id="customDialogOkay" class="btn z-depth-0">OK</button>': '') +
      '</div>';
